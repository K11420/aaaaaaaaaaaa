...
class AIPathfindingApp {
    constructor() {
        ...
        // ナビゲーション画像の経路線の太さ（ブラウザで変更可能）
        this.navigationLineWidth = 4;
    ...
    async intelligentAutoProcess() {
        ...
        try {
            ...
            const requestData = {
                ...
                target_size: 600  // 処理速度を優先（小さいほど高速）
            };
            ...
            if (response.data.success) {
                ...
                // AI分析結果を表示
                this.displayAIAnalysis(response.data.intelligent_analysis, response.data);
                ...
                if (response.data.road_network_preview) {
                    ...
                    // 道路ネットワークデータも保存（経路探索で使用）
                    this.roadNetworkData = {
                        nodes: response.data.road_network_preview.nodes || [],
                        skeleton_image: response.data.road_network_preview.skeleton_image
                    };
                    console.log('🛣️ 道路ネットワークプレビューデータ取得:', this.roadNetworkPreviewData);
                    
                    // 道路ネットワークを自動的に表示
                    this.showRoadNetwork = true;
                    const previewBtn = document.getElementById('previewRoadNetworkBtn');
                    if (previewBtn) {
                        previewBtn.textContent = '道路表示 OFF';
                    }
                    ...
                    // 道路ネットワークを描画
                    this.redrawPreviewWithRoadNetwork();
                } else {
    ...
    displayAIAnalysis(analysis, responseData = {}) {
        // AI分析結果セクションを表示
        ...
        // 詳細な処理時間を表示
        this.displayProcessingTimeDetails(timing, responseData.road_network_preview?.timing);
    }
    
    displayProcessingTimeDetails(timing, networkTiming) {
        // 処理時間詳細セクションを表示
        const detailsSection = document.getElementById('processingTimeDetails');
        if (!detailsSection) return;
        
        let html = '<div class="mt-4 p-4 bg-gray-50 rounded-lg">';
        html += '<h4 class="font-bold text-lg mb-3">⏱️ 処理時間の詳細</h4>';
        html += '<div class="space-y-2">';
        
        // 基本処理時間
        if (timing.analysis_time) {
            html += `<div class="flex justify-between"><span>📊 画像特徴分析:</span><span class="font-mono">${(timing.analysis_time * 1000).toFixed(0)}ms</span></div>`;
        }
        if (timing.resize_time) {
            html += `<div class="flex justify-between"><span>🖼️ 画像リサイズ:</span><span class="font-mono">${(timing.resize_time * 1000).toFixed(0)}ms</span></div>`;
        }
        if (timing.binarization_time) {
            html += `<div class="flex justify-between"><span>🎨 二値化処理:</span><span class="font-mono">${(timing.binarization_time * 1000).toFixed(0)}ms</span></div>`;
        }
        if (timing.noise_reduction_time) {
            html += `<div class="flex justify-between"><span>🧹 ノイズ除去:</span><span class="font-mono">${(timing.noise_reduction_time * 1000).toFixed(0)}ms</span></div>`;
        }
        if (timing.wall_thickening_time) {
            html += `<div class="flex justify-between"><span>🧱 壁を太くする処理:</span><span class="font-mono">${(timing.wall_thickening_time * 1000).toFixed(0)}ms</span></div>`;
        }
        if (timing.grid_generation_time) {
            html += `<div class="flex justify-between"><span>📊 グリッド生成:</span><span class="font-mono">${(timing.grid_generation_time * 1000).toFixed(0)}ms</span></div>`;
        }
        if (timing.pathfinding_choice_time) {
            html += `<div class="flex justify-between"><span>🔍 アルゴリズム選択:</span><span class="font-mono">${(timing.pathfinding_choice_time * 1000).toFixed(0)}ms</span></div>`;
        }
        
        // 道路ネットワーク生成時間
        if (networkTiming) {
            html += '<div class="mt-3 pt-3 border-t border-gray-300">';
            html += '<div class="font-semibold mb-2">🛣️ 道路ネットワーク生成:</div>';
            if (networkTiming.step1_extract_roads) {
                html += `<div class="flex justify-between text-sm ml-2"><span>  ステップ1（通路抽出）:</span><span class="font-mono">${(networkTiming.step1_extract_roads * 1000).toFixed(0)}ms</span></div>`;
            }
            if (networkTiming.step2_skeletonize) {
                html += `<div class="flex justify-between text-sm ml-2"><span>  ステップ2（スケルトン化）:</span><span class="font-mono">${(networkTiming.step2_skeletonize * 1000).toFixed(0)}ms</span></div>`;
            }
            if (networkTiming.step3_detect_nodes) {
                html += `<div class="flex justify-between text-sm ml-2"><span>  ステップ3（ノード検出）:</span><span class="font-mono">${(networkTiming.step3_detect_nodes * 1000).toFixed(0)}ms</span></div>`;
            }
            if (networkTiming.step4_generate_edges) {
                html += `<div class="flex justify-between text-sm ml-2"><span>  ステップ4（エッジ生成）:</span><span class="font-mono">${(networkTiming.step4_generate_edges * 1000).toFixed(0)}ms</span></div>`;
            }
            if (networkTiming.step5_create_grid) {
                html += `<div class="flex justify-between text-sm ml-2"><span>  ステップ5（グリッド生成）:</span><span class="font-mono">${(networkTiming.step5_create_grid * 1000).toFixed(0)}ms</span></div>`;
            }
            if (networkTiming.total_time) {
                html += `<div class="flex justify-between font-semibold mt-2 pt-2 border-t border-gray-200"><span>  合計:</span><span class="font-mono">${(networkTiming.total_time * 1000).toFixed(0)}ms</span></div>`;
            }
            html += '</div>';
        }
        ...
        if (timing.total_time) {
            html += '<div class="mt-3 pt-3 border-t-2 border-blue-300">';
            html += `<div class="flex justify-between font-bold text-lg"><span>⏱️ 総処理時間:</span><span class="font-mono text-blue-600">${(timing.total_time * 1000).toFixed(0)}ms</span></div>`;
            html += '</div>';
        }
    ...
    async findPath() {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                this.showSuccessMessage('🎉 経路検索が完了しました！結果をご確認ください。');
                ...
                // 道路ネットワークデータを保存
                if (response.data.road_network) {
                    this.roadNetworkData = response.data.road_network;
                    // 道路ネットワークボタンを表示
                    const roadNetworkBtn = document.getElementById('roadNetworkBtn');
                    if (roadNetworkBtn) {
                        roadNetworkBtn.style.display = 'block';
                    }
                }
            } else {
                console.error('❌ 経路検索失敗:', response.data.error);
                
                // 経路が見つからなくても、道路ネットワークがあれば表示
                if (response.data.road_network) {
                    this.roadNetworkData = response.data.road_network;
                    console.log('🛣️ 経路が見つかりませんでしたが、道路ネットワークを表示します');
                    ...
                    this.showRoadNetwork = true;
                    if (roadNetworkBtn) {
                        roadNetworkBtn.textContent = '道路表示 OFF';
                    }
                    this.redrawWithRoadNetwork();
                }
    ...
    displayPathResult(result) {
        ...
        // ナビゲーション画像（元画像＋経路、線の太さ変更可能）を表示
        this.displayNavigationImage(result);
        
        // 結果セクションまでスクロール
        document.getElementById('resultInfo').scrollIntoView({ behavior: 'smooth' });
    }
    
    displayNavigationImage(result) {
        // 元画像がない場合はスキップ
        if (!this.originalImageBase64 || !result.path || result.path.length === 0) return;
        
        let section = document.getElementById('navigationImageSection');
        if (!section) {
            section = document.createElement('div');
            section.id = 'navigationImageSection';
            section.className = 'mt-6 p-6 bg-white rounded-xl border border-gray-200';
            const resultInfo = document.getElementById('resultInfo');
            if (resultInfo && resultInfo.parentNode) {
                resultInfo.parentNode.insertBefore(section, resultInfo.nextSibling);
            } else {
                document.getElementById('processedImageContainer')?.parentNode?.appendChild(section);
            }
        }
        ...
        section.innerHTML = `
            <h3 class="text-xl font-bold text-gray-800 mb-4 flex items-center">
                <span class="mr-2">🗺️</span> ナビゲーション画像（元画像＋経路）
            </h3>
            <div class="mb-4 flex items-center gap-4 flex-wrap">
                <label class="font-medium text-gray-700">経路の線の太さ:</label>
                <input type="range" id="navigationLineWidthSlider" min="1" max="20" value="${this.navigationLineWidth}" class="w-40 h-2 accent-amber-500">
                <span id="navigationLineWidthValue" class="font-mono text-amber-600 font-bold">${this.navigationLineWidth}px</span>
            </div>
            <div id="navigationImageContainer" class="flex justify-center bg-gray-100 rounded-lg p-2 overflow-auto">
                <canvas id="navigationImageCanvas"></canvas>
            </div>
        `;
        
        const slider = document.getElementById('navigationLineWidthSlider');
        const valueSpan = document.getElementById('navigationLineWidthValue');
        if (slider) {
            slider.addEventListener('input', (e) => {
                this.navigationLineWidth = parseInt(e.target.value, 10);
                if (valueSpan) valueSpan.textContent = this.navigationLineWidth + 'px';
                this.redrawNavigationImage();
            });
        }
    ...
    redrawNavigationImage() {
        if (!this.originalImageBase64 || !this.lastResult || !this.lastResult.path || !this.currentGrid) return;
        
        const canvas = document.getElementById('navigationImageCanvas');
        if (!canvas) return;
        
        const ctx = canvas.getContext('2d');
        this._navCtx = ctx;
        
        const gridWidth = this.currentGrid[0].length;
        const gridHeight = this.currentGrid.length;
        ...
        img.onload = () => {
            ...
            this.drawPath(ctx, this.lastResult.path, null, this.navigationLineWidth);
        };
    ...
    drawPath(ctx, path, roadNetwork = null, lineWidth = null) {
        if (!path || path.length === 0) return;
        
        // 線の太さ（指定がなければナビゲーション用の設定を使用）
        const width = lineWidth != null ? lineWidth : (ctx === this._navCtx ? this.navigationLineWidth : 4);
    ...
    redrawWithRoadNetwork() {
        // 経路探索後の道路ネットワーク表示
        const canvas = document.querySelector('#processedImageContainer canvas');
        if (canvas && this.roadNetworkData) {
            const ctx = canvas.getContext('2d');
            ...
            if (this.currentProcessedImage) {
                ...
                img.onload = () => {
                    ...
                    if (this.showRoadNetwork) {
                        this.drawRoadNetwork(ctx, this.roadNetworkData);
                    }
                    // 経路があれば描画
                    if (this.lastResult && this.lastResult.path) {
    ...
    toggleRoadNetworkDisplay() {
        ...
        } else if (canvas && this.roadNetworkPreviewData) {
            // 経路がない場合はプレビュー道路ネットワークを表示
            this.redrawPreviewWithRoadNetwork();
        }
    }
    ...
    clearPath() {
        ...
        // ナビゲーション画像セクションを非表示
        const navSection = document.getElementById('navigationImageSection');
