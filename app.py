...
# アップロード画像保存先（リンク末尾のセッションIDをファイル名に使用）
...
class NavigationInstructionGenerator:
    ...
    def generate_navigation_instructions(self, path, original_image=None):
        """★改良版: 歩数なしでランドマーク間移動のナビゲーション指示生成"""
        ...
        # 開始地点のランドマークを検出
        start_landmark = self.landmark_detector.detect_landmarks_around_position(
            original_image, (path[0][0], path[0][1])
        ) if original_image is not None else "出発地点"
        
        # 最初の指示
        instructions.append({
            "step": 1,
            "distance": 0,
            "instruction": f"🚩 {start_landmark}からスタートしてください",
            "position": {"x": path[0][0], "y": path[0][1]},
            "direction": "開始",
        ...
        # 各キーポイント間でランドマークベースの指示を生成
        for i, (point_idx, point_coord) in enumerate(key_points):
            if i == len(key_points) - 1:
                continue  # 最後のポイントは終了地点として別処理
            
            # 現在地点と次の地点のランドマークを検出
            current_landmark = self.landmark_detector.detect_landmarks_around_position(
                original_image, point_coord
            ) if original_image is not None else f"地点{i+2}"
            
            next_point_coord = key_points[i + 1][1]
            next_landmark = self.landmark_detector.detect_landmarks_around_position(
                original_image, next_point_coord
            ) if original_image is not None else f"地点{i+3}"
            ...
            # ランドマーク間の移動指示を生成（歩数なし）
            if turn == "直進":
                if current_landmark == next_landmark:
                    instruction_text = f"📍 {current_landmark}をそのまま直進してください"
                else:
                    instruction_text = f"📍 {current_landmark}から{next_landmark}方向へ直進してください"
            elif turn == "右折":
                instruction_text = f"📍 {current_landmark}で右に曲がって{next_landmark}方向へ進んでください"
            elif turn == "左折":
                instruction_text = f"📍 {current_landmark}で左に曲がって{next_landmark}方向へ進んでください"
            elif turn == "Uターン":
                instruction_text = f"📍 {current_landmark}でUターンして{next_landmark}方向へ進んでください"
            else:
                instruction_text = f"📍 {current_landmark}から{next_landmark}方向へ進んでください"
            
            instructions.append({
                "step": len(instructions) + 1,
                "distance": 0,  # 歩数は表示しない
                "instruction": instruction_text,
                "position": {"x": point_coord[0], "y": point_coord[1]},
                "direction": direction,
                "turn": turn,
        ...
        # 最終指示 - 目的地のランドマーク
        end_landmark = self.landmark_detector.detect_landmarks_around_position(
            original_image, (path[-1][0], path[-1][1])
        ) if original_image is not None else "目的地"
        
        instructions.append({
            "step": len(instructions) + 1,
            "distance": 0,
            "instruction": f"🏁 {end_landmark}に到着しました！",
            "position": {"x": path[-1][0], "y": path[-1][1]},
            "direction": "到着",
    ...
    def identify_key_navigation_points(self, path):
        ...
        for i in range(1, len(path) - 1):
            ...
            # 現在の方向
            direction = self.calculate_direction(prev_point, current_point)
            next_direction = self.calculate_direction(current_point, next_point)
            
            # 方向が変わった場合はキーポイントとして記録
            if current_direction is None:
                current_direction = direction
            elif direction != current_direction or direction != next_direction:
                # 最小距離チェック（近すぎるポイントは統合）
                last_key_point = key_points[-1][1]
                distance = abs(current_point[0] - last_key_point[0]) + abs(current_point[1] - last_key_point[1])
                
                if distance > 20:  # 最小距離20ピクセル
...
class RoadNetworkGenerator:
    ...
    def generate_road_network(self, binary_image: np.ndarray) -> Dict[str, Any]:
        ...
        print(f"  ⏱️ ステップ1（通路抽出）: {time.time() - step1_start:.3f}秒")
        
        # 2. スケルトン化による道路中央線の抽出
        step2_start = time.time()
        skeleton = self.extract_road_skeleton(road_mask)
        print(f"  ⏱️ ステップ2（スケルトン化）: {time.time() - step2_start:.3f}秒")
        
        # 3. ノード候補の検出
        step3_start = time.time()
        nodes = self.detect_road_nodes(skeleton)
        print(f"  ⏱️ ステップ3（ノード検出）: {time.time() - step3_start:.3f}秒")
        
        # 4. エッジの生成（壁チェック付き）
        step4_start = time.time()
        edges = self.generate_edges(nodes, skeleton, binary_image)
        print(f"  ⏱️ ステップ4（エッジ生成）: {time.time() - step4_start:.3f}秒")
        
        # 5. 道路グリッドの生成
        step5_start = time.time()
        road_grid = self.create_road_grid(skeleton, binary_image.shape)
        print(f"  ⏱️ ステップ5（グリッド生成）: {time.time() - step5_start:.3f}秒")
        
        total_network_time = time.time() - network_start
        step1_time = time.time() - step1_start
        step2_time = time.time() - step2_start
        step3_time = time.time() - step3_start
        step4_time = time.time() - step4_start
        step5_time = time.time() - step5_start
        print(f"✅ ノード数: {len(nodes)}, エッジ数: {len(edges)}, 総時間: {total_network_time:.3f}秒")
        ...
        return {
            ...
            'timing': {
                'step1_extract_roads': step1_time,
                'step2_skeletonize': step2_time,
                'step3_detect_nodes': step3_time,
                'step4_generate_edges': step4_time,
                'step5_create_grid': step5_time,
                'total_time': total_network_time
            }
        }
    
    def extract_road_skeleton(self, road_mask: np.ndarray) -> np.ndarray:
        """道路の中央線をスケルトン化で抽出（高速化版）"""
        
        # モルフォロジー演算でノイズ除去（高速化：小さいカーネル）
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        cleaned = cv2.morphologyEx(road_mask, cv2.MORPH_CLOSE, kernel)
        
        # スケルトン化（細線化）- thin()は省略して高速化
        skeleton = skeletonize(cleaned > 0).astype(np.uint8) * 255
        
        # スケルトン線は元の細さを維持（輪郭線は別途太くする）
        # スケルトン線を1px太くする（軽微な補正のみ）
        thicken_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        skeleton = cv2.dilate(skeleton, thicken_kernel, iterations=1)
    ...
    def detect_road_nodes(self, skeleton: np.ndarray) -> List[Tuple[int, int, int]]:
        ...
        # 高速化: サンプリングしてからクラスタリング
        max_points = 5000  # 最大ポイント数を制限
        if len(skeleton_points) > max_points:
            indices = np.random.choice(len(skeleton_points), max_points, replace=False)
        ...
        clusters = clustering.fit_predict(sampled_points)
        ...
        for cluster_id in set(clusters):
            ...
            cluster_points = sampled_points[clusters == cluster_id]
    ...
    def generate_edges(self, nodes: List, skeleton: np.ndarray, binary_mask: np.ndarray = None) -> List[Tuple[int, int, float]]:
        """ノード間のエッジを生成（壁チェック付き）"""
        edges = []
        ...
        for i, (x1, y1, id1) in enumerate(nodes):
            for j, (x2, y2, id2) in enumerate(nodes):
                ...
                if distance < self.node_spacing * 5:
                    # スケルトン接続チェック
                    if self.is_connected_on_skeleton(skeleton, (x1, y1), (x2, y2)):
                        # 壁チェック（binary_maskが提供されている場合）
                        if binary_mask is not None:
                            if not self.edge_crosses_wall(binary_mask, (x1, y1), (x2, y2)):
        ...
        if len(edges) < len(nodes) * 0.5:
            print(f"⚠️ エッジ数が少ない（{len(edges)}）、距離ベースのエッジを追加")
            for i, (x1, y1, id1) in enumerate(nodes):
                for j, (x2, y2, id2) in enumerate(nodes):
                    if i >= j:
                        continue
    ...
    def edge_crosses_wall(self, binary_mask: np.ndarray, point1: Tuple[int, int], point2: Tuple[int, int]) -> bool:
        """エッジが壁を横切るかチェック"""
        x1, y1 = point1
        x2, y2 = point2
    ...
    def is_connected_on_skeleton(self, skeleton: np.ndarray, point1: Tuple[int, int], 
        ...
        for x, y in points:
            if 0 <= x < skeleton.shape[1] and 0 <= y < skeleton.shape[0]:
                if skeleton[y, x] == 255:  # スケルトン上の白い点
                    skeleton_point_count += 1
        
        # 経路の80%以上がスケルトン上にあれば接続されているとみなす
        return skeleton_point_count >= total_points * 0.8
    ...
    def find_nearest_road_point_with_astar(self, start_point: Tuple[int, int], 
                                          skeleton: np.ndarray, grid: List[List[int]]) -> Tuple[int, int, List[Tuple[int, int]]]:
        """
        A*アルゴリズムで壁を避けながらスケルトンネットワークに接続
        ...
        # スケルトンポイントのセットを作成（高速検索用）
        skeleton_set = set((int(sx), int(sy)) for sy, sx in skeleton_points)
        
        # ヒューリスティック関数（マンハッタン距離）
        def heuristic(a: Tuple[int, int], b: Tuple[int, int]) -> float:
            return abs(a[0] - b[0]) + abs(a[1] - b[1])
        
        # 最も近いスケルトンポイントを目標として設定
        distances = np.sqrt(np.sum((skeleton_points - [y, x])**2, axis=1))
        nearest_idx = np.argmin(distances)
        target_y, target_x = skeleton_points[nearest_idx]
        ...
        f_score = {start_point: heuristic(start_point, target)}
        ...
        visited = set()
        max_iterations = width * height  # 無限ループ防止
        iteration = 0
        ...
        while open_set and iteration < max_iterations:
            ...
            # スケルトンポイントに到達したか、近く（5ピクセル以内）に到達したか
            if current in skeleton_set:
                # パスを再構築
                path = []
                ...
                path = path[::-1]
                print(f"✅ スケルトン接続成功: {start_point} → {path[-1]} ({len(path)}ステップ)")
                return path[-1], path
            
            # 目標の近く（5ピクセル以内）に到達した場合
            if abs(current[0] - target[0]) <= 5 and abs(current[1] - target[1]) <= 5:
                if current in skeleton_set:
                    path = []
                    while current in came_from:
                        path.append(current)
                    ...
                    path.append(start_point)
                    path = path[::-1]
                    print(f"✅ スケルトン接続成功（近接）: {start_point} → {path[-1]} ({len(path)}ステップ)")
                    return path[-1], path
                # 目標点に直接接続
                path = []
                temp = current
                ...
                path = path[::-1]
                path.append(target)
                print(f"✅ スケルトン接続成功（目標到達）: {start_point} → {target} ({len(path)}ステップ)")
                return target, path
            ...
            for dx, dy, cost in directions:
                ...
                if (0 <= neighbor[0] < width and 0 <= neighbor[1] < height and
                    grid[neighbor[1]][neighbor[0]] == 0):  # 通行可能
        ...
        # 接続失敗時は最も近いスケルトンポイントを返す
        print(f"⚠️ スケルトン接続失敗、最も近い点を使用")
...
class OptimizedPathfinder:
    ...
    def find_path_with_road_network(self, grid: List[List[int]], start: Tuple[int, int], 
        ...
        print(f"🛣️ 道路ネットワーク生成結果: ノード数={len(nodes)}, エッジ数={len(edges)}, スケルトン形状={skeleton.shape if skeleton is not None else 'None'}")
        
        if not nodes:
            # フォールバック: 従来の方法
            print("⚠️ 道路ネットワーク生成失敗、グリッドベース経路探索に切り替えます")
        ...
        print(f"📍 ノード経路長: {len(path_nodes) if path_nodes else 0}")
        
        if not path_nodes or len(path_nodes) == 0:
            print("❌ ノードグラフでの経路探索に失敗、スケルトン上直接経路を生成")
            # スケルトン上で直接A*経路探索を実行
            direct_path = self.pathfind_on_skeleton(skeleton, start_road, end_road, algorithm)
            
            if direct_path and len(direct_path) > 0:
                print(f"✅ スケルトン直接経路成功: {len(direct_path)}ステップ")
                # 開始点から道路への接続と道路から終了点への接続を追加
    ...
    def fallback_pathfinding(self, grid: List[List[int]], start: Tuple[int, int], 
        ...
        """フォールバック: 従来の経路探索"""
        if algorithm == 'BFS':
            path = self.bfs_pathfinding(grid, start, end)
        elif algorithm == 'A*':
            path = self.astar_pathfinding(grid, start, end)
        elif algorithm == 'Dijkstra':
            path = self.dijkstra_pathfinding(grid, start, end)
        else:
            path = self.bfs_pathfinding(grid, start, end)
...
class ImageProcessor:
    ...
    def intelligent_auto_process(self, image: np.ndarray, target_size: int = 700) -> Dict[str, Any]:
        ...
        print(f"📊 画像特徴分析: {analysis_time:.3f}秒", flush=True)
        ...
        print(f"🖼️ 画像リサイズ ({width}x{height} → {new_width}x{new_height}): {resize_time:.3f}秒", flush=True)
        ...
        print(f"🎨 二値化処理 ({method}): {binarization_time:.3f}秒", flush=True)
        ...
        # 小さなオブジェクト除去はスキップ（高速化のため）
        noise_time = time.time() - noise_start
        print(f"🧹 ノイズ除去（高速化）: {noise_time:.3f}秒", flush=True)
        
        # 5.5. 壁を太くする処理（黒い部分を膨張）- 輪郭線を太くする
        wall_thickening_start = time.time()
        inverted = cv2.bitwise_not(binary_image)
        wall_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
        dilated_walls = cv2.dilate(inverted, wall_kernel, iterations=2)  # iterations=1→2に増加
        binary_image = cv2.bitwise_not(dilated_walls)
        wall_thickening_time = time.time() - wall_thickening_start
        print(f"🧱 壁を太くする処理（輪郭線強化）: {wall_thickening_time:.3f}秒", flush=True)
        
        # 6. グリッド生成（高速化：NumPy操作を最適化）
        grid_start = time.time()
        # NumPy配列として処理してからリストに変換（高速）
        grid_array = (binary_image == 0).astype(int)
        grid = grid_array.tolist()
        grid_time = time.time() - grid_start
        print(f"📊 グリッド生成（高速化）: {grid_time:.3f}秒", flush=True)
        
        # 7. 最適な経路探索アルゴリズム選択
        pathfinding_choice_start = time.time()
        pathfinding_choice = self.selector.select_optimal_pathfinding(grid, (new_width, new_height))
        pathfinding_choice_time = time.time() - pathfinding_choice_start
        print(f"🔍 経路探索アルゴリズム選択: {pathfinding_choice_time:.3f}秒", flush=True)
        
        total_time = time.time() - start_time
        print(f"⏱️ 総処理時間: {total_time:.3f}秒", flush=True)
        ...
        return {
            ...
            'analysis': {
                ...
                'timing': {
                    ...
                    'wall_thickening_time': wall_thickening_time,
                    'grid_generation_time': grid_time,
                    'pathfinding_choice_time': pathfinding_choice_time,
...
def get_session(session_id):
    """セッションデータを取得"""
    if session_id in processed_sessions:
        return jsonify({'success': True, 'data': processed_sessions[session_id]})
...
def intelligent_process_image():
    ...
    try:
        ...
        # 目標サイズを最適化（処理速度とのバランス）
        target_size = int(data.get('target_size', 600))  # 処理速度を優先（小さいほど高速）
        ...
        # セッションIDを先に生成（ファイル名＝リンク末尾に使う）
        session_id = generate_session_id()
        
        # アップロード画像をそのままファイル保存（名前＝リンク末尾のセッションID）
        ...
        if ',' in data_url:
            ...
            save_path = os.path.join(UPLOADS_FOLDER, f'{session_id}.{ext}')
            with open(save_path, 'wb') as f:
                f.write(raw_bytes)
            original_image_url = f'/uploads/{session_id}.{ext}'
        else:
            original_image_url = None
        
        # 🛣️ スケルトン画像のみを高速生成（ノード・エッジ生成はスキップ）
        network_preview_start = time.time()
        binary_array = result['processed_image']
        try:
            # スケルトンのみ生成（高速）
            road_mask = (binary_array == 255).astype(np.uint8)
            skeleton = pathfinder.road_network_gen.extract_road_skeleton(road_mask)
            network_preview_time = time.time() - network_preview_start
            print(f"🛣️ スケルトン生成（高速）: {network_preview_time:.3f}秒", flush=True)
            
            road_network_preview = {
                'nodes': [],
                'nodes_count': 0,
                'timing': {'skeleton_only': network_preview_time}
            }
            ...
            if skeleton is not None:
                ...
                print(f"✅ プレビュー用スケルトン画像生成: {skeleton.shape}", flush=True)
            else:
                road_network_preview['skeleton_image'] = None
        except Exception as e:
            print(f"⚠️ スケルトン生成エラー: {e}", flush=True)
            road_network_preview = None
        
        # セッション保存（元画像はURLで参照＝リンク末尾のファイル名）
...
def find_path():
    ...
    try:
        ...
        # 🛣️ 道路ネットワークベースの経路探索
        # 注意: 道路ネットワーク生成は重いため、デフォルトで無効化
        # スケルトン表示は事前生成したものを使用
        use_road_network = data.get('use_road_network', False)  # デフォルトで従来アルゴリズム使用（高速）
        ...
        if use_road_network:
            ...
            try:
                ...
                if skeleton is not None:
                    print(f"🛤️ スケルトンネットワーク上で経路探索を開始")
                    
                    # 1. 始点からスケルトンへのA*接続
                    start_road, start_connection_path = pathfinder.road_network_gen.find_nearest_road_point_with_astar(
                        start_point, skeleton, grid
                    )
                    
                    # 2. 終点からスケルトンへのA*接続
                    end_road, end_connection_path = pathfinder.road_network_gen.find_nearest_road_point_with_astar(
                        end_point, skeleton, grid
                    )
                    
                    print(f"🎯 接続点: 始点→{start_road}, 終点→{end_road}")
                    
                    # 3. スケルトン上でのA*経路探索
                    skeleton_path = pathfinder.pathfind_on_skeleton(skeleton, start_road, end_road, algorithm)
                    
                    if skeleton_path and len(skeleton_path) > 0:
                        # 経路を結合: 始点接続 → スケルトン経路 → 終点接続
                        full_path = []
                        
                        # 始点接続経路（重複を避ける）
                        if start_connection_path:
                            full_path.extend(start_connection_path[:-1])  # 最後の点（start_road）は除く
                        else:
                            full_path.append(start_point)
                        
                        # スケルトン経路
                        full_path.extend(skeleton_path)
                        
                        # 終点接続経路（逆順、重複を避ける）
                        if end_connection_path:
                            # end_roadから終点への経路を逆順で追加
                            for i in range(len(end_connection_path) - 2, -1, -1):  # end_roadを除く
                                full_path.append(end_connection_path[i])
                        else:
                            full_path.append(end_point)
                        
                        path = full_path
                        print(f"✅ スケルトンネットワーク経路完成: {len(path)}ポイント（接続: {len(start_connection_path) if start_connection_path else 0}+{len(skeleton_path)}+{len(end_connection_path) if end_connection_path else 0}）")
                    else:
                        print(f"⚠️ スケルトン上での経路探索失敗、元の経路を使用")
                        # 元の経路を使用
                ...
                road_network_data = {
                    'nodes': [],
                    'edges': [],
                    'start_road': start_road if skeleton is not None else None,
                    'end_road': end_road if skeleton is not None else None,
                    'road_path': skeleton_path if skeleton is not None else [],
                    'skeleton_image': f"data:image/png;base64,{skeleton_b64}"
                }
                print(f"✅ スケルトン画像生成完了: {skeleton.shape}")
            except Exception as e:
                print(f"⚠️ スケルトン生成エラー: {e}")
                import traceback
                traceback.print_exc()
                road_network_data = None
        ...
        if path and len(path) > 0:
            ...
            # ナビゲーション画像用にクライアントが送った元画像をそのまま返す
            if data.get('original_image_base64'):
                response_data['original_image_base64'] = data['original_image_base64']
            ...
            if road_network_data:
                # 既にskeleton_imageが含まれている場合（従来アルゴリズム用）
                if 'skeleton_image' in road_network_data:
                    response_data['road_network'] = {
                        'nodes': road_network_data.get('nodes', []),
                        'edges_count': len(road_network_data.get('edges', [])),
                        'start_road': road_network_data.get('start_road'),
                        'end_road': road_network_data.get('end_road'),
                        'road_path': road_network_data.get('road_path', []),
                        'skeleton_image': road_network_data['skeleton_image']
                    }
                    print("✅ スケルトン画像を追加（従来アルゴリズム用）")
                else:
                    response_data['road_network'] = {
...
def uploads_files(filename):
    """アップロード画像配信（リンク末尾のセッションIDで保存したファイル）"""
